...
export function ProductCard({
  ...
  return (
    <div className="group relative overflow-hidden rounded-lg border border-white/[0.06] bg-white/[0.03] transition-colors hover:border-emerald/40">
      <Link href={productPath(product)} className="block">
        <div className="relative aspect-square overflow-hidden bg-white/5">
          ...
          {tag ? (
            <span className="absolute bottom-2 left-2 rounded-full bg-gold/90 px-2.5 py-1 font-label text-[11px] font-bold text-navy-deep">
              {tag}
            </span>
          ) : null}
        </div>
      ...
      {/* Hors du lien : un bouton ne peut pas être imbriqué dans un <a>. */}
